...
agent_communication:
  ...
  - agent: "main"
    message: "Fixed Pull-to-Refresh issue. The app was refreshing the page every time user tried to scroll up because RefreshControl was always enabled. Implemented conditional refresh: 1) Added isAtTop state to track WebView scroll position, 2) Added onScroll handler to WebView that updates isAtTop based on contentOffset.y, 3) Set RefreshControl enabled={isAtTop} so refresh only triggers when at top of page, 4) Added scrollEventThrottle={16} for smooth position tracking, 5) Reset isAtTop to true on new page load. Now pull-to-refresh only works when user is at the top of the page."